...
export class TelephoneNumberComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.registerChangeInTelephoneNumbers();
  }
  ...
  trackId(index: number, item: ITelephoneNumber) {
    return item.id;
  }

  registerChangeInTelephoneNumbers() {
    this.eventSubscriber = this.eventManager.subscribe('telephoneNumberListModification', response => this.loadAll());
  }
  ...
  protected paginateTelephoneNumbers(data: ITelephoneNumber[], headers: HttpHeaders) {
