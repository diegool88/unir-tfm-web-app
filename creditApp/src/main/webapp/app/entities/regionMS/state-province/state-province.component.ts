...
export class StateProvinceComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.stateProvinceService
      ...
      .subscribe(
        (res: HttpResponse<IStateProvince[]>) => this.paginateStateProvinces(res.body, res.headers),
  ...
  transition() {
    this.router.navigate(['/state-province'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: IStateProvince) {
    return item.id;
  }

  registerChangeInStateProvinces() {
    this.eventSubscriber = this.eventManager.subscribe('stateProvinceListModification', response => this.loadAll());
  }
  ...
  protected paginateStateProvinces(data: IStateProvince[], headers: HttpHeaders) {
