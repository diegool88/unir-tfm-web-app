...
import com.dfgtech.tfm.regionms.repository.CityRepository;
import com.dfgtech.tfm.regionms.service.CityService;
import com.dfgtech.tfm.regionms.service.dto.CityDTO;
import com.dfgtech.tfm.regionms.service.mapper.CityMapper;
...
public class CityResourceIT {
    ...
    public static City createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, StateProvince.class).isEmpty()) {
            stateProvince = StateProvinceResourceIT.createEntity(em);
            em.persist(stateProvince);
            em.flush();
        } else {
            stateProvince = TestUtil.findAll(em, StateProvince.class).get(0);
        }
        city.setState(stateProvince);
        return city;
    }
    /**
    ...
    public static City createUpdatedEntity(EntityManager em) {
        ...
        StateProvince stateProvince;
        if (TestUtil.findAll(em, StateProvince.class).isEmpty()) {
            stateProvince = StateProvinceResourceIT.createUpdatedEntity(em);
            em.persist(stateProvince);
            em.flush();
        } else {
            stateProvince = TestUtil.findAll(em, StateProvince.class).get(0);
